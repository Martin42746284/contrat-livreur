...
const PartyForm: React.FC<PartyFormProps> = ({ type, data, onChange, onSave, onVerifyCIN, readOnly }) => {
    ...
    <div className={`rounded-xl border p-6 space-y-4 animate-fade-in ${readOnly ? 'bg-muted/50 border-border' : 'bg-card border-accent/30 shadow-contract'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-1.5">
          ...
          <Input value={data.nomComplet} onChange={e => onChange({ nomComplet: e.target.value })} onBlur={onSave} disabled={readOnly} placeholder="Nom et prénom" />
        </div>
        <div className="space-y-1.5">
          <Label>CIN *</Label>
          <Input
        ...
        <div className="space-y-1.5">
          <Label>Adresse</Label>
          <Input value={data.adresse} onChange={e => onChange({ adresse: e.target.value })} onBlur={onSave} disabled={readOnly} placeholder="Adresse complète" />
        </div>
        <div className="space-y-1.5">
        ...
        {!isFournisseuse && (
          <div className="space-y-1.5 md:col-span-2">
            ...
            <Input value={(data as DistributorInfo).nomPage || ''} onChange={e => onChange({ nomPage: e.target.value })} onBlur={onSave} disabled={readOnly} placeholder="Nom de la boutique en ligne" />
          </div>
        )}
      </div>
